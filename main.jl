...
αs = [0.95 .* rand(24) for _ in 1:5]
δs = [100 .+ rand(24) .* 300 for _ in 1:5]
ubs = [[(thermal = 1e3, res = 1e3) for _ in 1:24]]

for k in 1:50
    m_thermal = make_thermal(prices, ubs)
    m_res = make_res(αs, prices, ubs)

    SDDP.train(m_thermal; print_level = 0, run_numerical_stability_report = false, stopping_rules = [SDDP.FirstStageStoppingRule(; atol=1e-1, iterations=5)])
    SDDP.train(m_res; print_level = 0, run_numerical_stability_report = false, stopping_rules = [SDDP.FirstStageStoppingRule(; atol=1e-1, iterations=5)])
    ...
    det = SDDP.deterministic_equivalent(make_storage(prices), () -> Gurobi.Optimizer(GRB_ENV))
    set_silent(det)
    optimize!(det)
    ...
    for t in 1:24
        ...
        if gen < 0
            # Consumption.
            push!(bids, (gen, avg_price + reduced_cost(variable_by_name(det, "discharge[$(t)]#1"))))
        elseif gen > 0
            # Generation.
            push!(bids, (gen, avg_price - reduced_cost(variable_by_name(det, "charge[$(t)]#1"))))
        else
            push!(bids, (0, 0))
        end
    end
    ...
    @constraint(m_market, dc[t = 1:24], sum(award[g, t] for g in [:thermal, :res, :storage, :slack]) == δ[t])
    @objective(m_market, Min, sum(1000.0 * award[:slack, t] + VOM_THERMAL * award[:thermal, t] + VOM_RES * award[:res, t] + bids[t][2] * award[:storage, t] for t in 1:24))
    optimize!(m_market)
    λ = -shadow_price.(dc)

    ub = []
    for t in 1:24
        max_res_award = value(award[:res, t]) + value(award[:thermal, t]) + value(award[:slack, t]) + (bids[t][2] > VOM_RES && bids[t][1] > 0 ? value(award[:storage, t]) : 0.0)
        max_thermal_award = value(award[:thermal, t]) + value(award[:slack, t]) + (bids[t][2] > VOM_THERMAL && bids[t][1] > 0 ? value(award[:storage, t]) : 0.0) 
    ...
    elseif (k <= 10) && (k % 10 == 0)
        ...
        ubs = [[(thermal = sum(el[t].thermal for el in ubs)/length(ubs), res = sum(el[t].res for el in ubs)/length(ubs)) for t in 1:24]]
    else
        push!(prices, λ)
        push!(ubs, ub)
    end
